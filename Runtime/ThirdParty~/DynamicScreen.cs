...
namespace libVT100
{
    ...
    /// </summary>
    public class DynamicScreen : IAnsiDecoderClient, IEnumerable<Screen.Character>
    {
        /// <summary>
        ...
        public class Run
        {
            ...
            public Screen.GraphicAttributes Attributes { get; set; }
        ...
        public class Line
        {
            ...
            /// </summary>
            public Screen.Character this[int col]
            {
                get { return CharacterFromColumn(col); }
                set
                {
                    ...
                    else
                    {
                        ...
                        if (run > Runs.Count)
                        {
                            throw new ArgumentOutOfRangeException(
                                $"The run ({run}) is larger than the number of runs ({Runs.Count})");
                        }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                var splitRun = new Run()
                                    { Attributes = Runs[run].Attributes, Length = splitStart - 1, Start = col + 1 };
            ...
            public Screen.Character CharacterFromColumn(int col)
            {
                var run = RunFromColumn(col);
                if (run != null)
                {
                    return new Screen.Character((col < Text.Length) ? Text[col] : (char)0)
            ...
            protected void CheckColumn(int column)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException(
                        $"The column number ({column}) is larger than the width ({Text.Length})");
                }
            }
        }
        ...
        protected Screen.GraphicAttributes _currentAttributes;
        ...
        public Screen.Character this[int column, int row]
        {
            get
            {
                CheckColumn(column);
        ...
        protected void CheckColumn(int column)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    $"The column number ({column}) is larger than the width ({Width})");
            }
        }
        ...
        protected void CheckRow(int row)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(
                    $"The row number ({row}) is larger than the number of lines ({Lines.Count})");
            }
        }
        ...
        void IAnsiDecoderClient.Characters(IAnsiDecoder _sender, char[] _chars)
        {
            foreach (char ch in _chars)
            {
                ...
                else if (ch == '\t' && TabSpaces > 0)
                {
                    ...
                    {
                        int start = this[CursorPosition.Y].Runs.Count == 0
                            ? 0
                            : Lines[CursorPosition.Y].Runs.Sum(r => r.Length);
                        Lines[CursorPosition.Y].Runs.Add(new Run()
                            { Attributes = _currentAttributes, Start = start, HasTab = true });
        ...
        void IAnsiDecoderClient.MoveCursorToBeginningOfLineBelow(IAnsiDecoder _sender,
            int _lineNumberRelativeToCurrentLine)
        {
            _cursorPosition.X = 0;
        ...
        void IAnsiDecoderClient.MoveCursorToBeginningOfLineAbove(IAnsiDecoder _sender,
            int _lineNumberRelativeToCurrentLine)
        {
            _cursorPosition.X = 0;
            while (_lineNumberRelativeToCurrentLine > 0)
        ...
        void IAnsiDecoderClient.ModeChanged(IAnsiDecoder _sender, AnsiMode _mode)
        {
            switch (_mode)
            {
                ...
                case AnsiMode.ShowCursor:
                    _showCursor = true;
                    break;
                case AnsiMode.OpenBracketedPaste:
                    break;
                case AnsiMode.CloseBracketedPaste:
                    break;
            }
        }
            ...
            for (var i = 0; i < _commands.Length; i++)
            {
                switch (_commands[i])
                {
                    ...
                    case GraphicRendition.Underline:
                        _currentAttributes.Underline = Screen.Underline.Single;
                        break;
                    case GraphicRendition.BlinkSlow:
                        _currentAttributes.Blink = Screen.Blink.Slow;
                        break;
                    case GraphicRendition.BlinkRapid:
                        _currentAttributes.Blink = Screen.Blink.Rapid;
                        break;
                    case GraphicRendition.Positive:
                    case GraphicRendition.Inverse:
                    {
                        Screen.TextColor tmp = _currentAttributes.Foreground;
                    ...
                    case GraphicRendition.UnderlineDouble:
                        _currentAttributes.Underline = Screen.Underline.Double;
                        break;
                    ...
                    case GraphicRendition.NoUnderline:
                        _currentAttributes.Underline = Screen.Underline.None;
                        break;
                    case GraphicRendition.NoBlink:
                        _currentAttributes.Blink = Screen.Blink.None;
                        break;
                    case GraphicRendition.Reveal:
                        _currentAttributes.Conceal = false;
                        break;
                    case GraphicRendition.ForegroundNormalBlack:
                        _currentAttributes.Foreground = Screen.TextColor.Black;
                        break;
                    case GraphicRendition.ForegroundNormalRed:
                        _currentAttributes.Foreground = Screen.TextColor.Red;
                        break;
                    case GraphicRendition.ForegroundNormalGreen:
                        _currentAttributes.Foreground = Screen.TextColor.Green;
                        break;
                    case GraphicRendition.ForegroundNormalYellow:
                        _currentAttributes.Foreground = Screen.TextColor.Yellow;
                        break;
                    case GraphicRendition.ForegroundNormalBlue:
                        _currentAttributes.Foreground = Screen.TextColor.Blue;
                        break;
                    case GraphicRendition.ForegroundNormalMagenta:
                        _currentAttributes.Foreground = Screen.TextColor.Magenta;
                        break;
                    case GraphicRendition.ForegroundNormalCyan:
                        _currentAttributes.Foreground = Screen.TextColor.Cyan;
                        break;
                    case GraphicRendition.ForegroundNormalWhite:
                        _currentAttributes.Foreground = Screen.TextColor.White;
                        break;
                    ...
                    case GraphicRendition.BackgroundNormalBlack:
                        _currentAttributes.Background = Screen.TextColor.Black;
                        break;
                    case GraphicRendition.BackgroundNormalRed:
                        _currentAttributes.Background = Screen.TextColor.Red;
                        break;
                    case GraphicRendition.BackgroundNormalGreen:
                        _currentAttributes.Background = Screen.TextColor.Green;
                        break;
                    case GraphicRendition.BackgroundNormalYellow:
                        _currentAttributes.Background = Screen.TextColor.Yellow;
                        break;
                    case GraphicRendition.BackgroundNormalBlue:
                        _currentAttributes.Background = Screen.TextColor.Blue;
                        break;
                    case GraphicRendition.BackgroundNormalMagenta:
                        _currentAttributes.Background = Screen.TextColor.Magenta;
                        break;
                    case GraphicRendition.BackgroundNormalCyan:
                        _currentAttributes.Background = Screen.TextColor.Cyan;
                        break;
                    case GraphicRendition.BackgroundNormalWhite:
                        _currentAttributes.Background = Screen.TextColor.White;
                        break;
                    ...
                    case GraphicRendition.ForegroundBrightBlack:
                        _currentAttributes.Foreground = Screen.TextColor.BrightBlack;
                        break;
                    case GraphicRendition.ForegroundBrightRed:
                        _currentAttributes.Foreground = Screen.TextColor.BrightRed;
                        break;
                    case GraphicRendition.ForegroundBrightGreen:
                        _currentAttributes.Foreground = Screen.TextColor.BrightGreen;
                        break;
                    case GraphicRendition.ForegroundBrightYellow:
                        _currentAttributes.Foreground = Screen.TextColor.BrightYellow;
                        break;
                    case GraphicRendition.ForegroundBrightBlue:
                        _currentAttributes.Foreground = Screen.TextColor.BrightBlue;
                        break;
                    case GraphicRendition.ForegroundBrightMagenta:
                        _currentAttributes.Foreground = Screen.TextColor.BrightMagenta;
                        break;
                    case GraphicRendition.ForegroundBrightCyan:
                        _currentAttributes.Foreground = Screen.TextColor.BrightCyan;
                        break;
                    case GraphicRendition.ForegroundBrightWhite:
                        _currentAttributes.Foreground = Screen.TextColor.BrightWhite;
                        break;
                    ...
                    case GraphicRendition.BackgroundBrightBlack:
                        _currentAttributes.Background = Screen.TextColor.BrightBlack;
                        break;
                    case GraphicRendition.BackgroundBrightRed:
                        _currentAttributes.Background = Screen.TextColor.BrightRed;
                        break;
                    case GraphicRendition.BackgroundBrightGreen:
                        _currentAttributes.Background = Screen.TextColor.BrightGreen;
                        break;
                    case GraphicRendition.BackgroundBrightYellow:
                        _currentAttributes.Background = Screen.TextColor.BrightYellow;
                        break;
                    case GraphicRendition.BackgroundBrightBlue:
                        _currentAttributes.Background = Screen.TextColor.BrightBlue;
                        break;
                    case GraphicRendition.BackgroundBrightMagenta:
                        _currentAttributes.Background = Screen.TextColor.BrightMagenta;
                        break;
                    case GraphicRendition.BackgroundBrightCyan:
                        _currentAttributes.Background = Screen.TextColor.BrightCyan;
                        break;
                    case GraphicRendition.BackgroundBrightWhite:
                        _currentAttributes.Background = Screen.TextColor.BrightWhite;
                        break;
                    ...
                    case GraphicRendition.ForegroundColor:
                        ...
                        _currentAttributes.ForegroundColor =
                            new Color((int)_commands[2], (int)_commands[3], (int)_commands[4]);
                        i += 4;
                        break;
                    ...
                    case GraphicRendition.BackgroundColor:
                        ...
                        /// Next arguments are 5;n or 2;r;g;b, see colors
                        /// _commands is [38, 2, r, g, b, ...] 
                        _currentAttributes.BackgroundColor =
                            new Color((int)_commands[2], (int)_commands[3], (int)_commands[4]);
                        i += 4;
                        break;
